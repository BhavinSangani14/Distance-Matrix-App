...
def get_distance_matrix(api_key, origin_list, destination_list, transport_mode):
    ...
    max_size = len(origin_list)//batch_size
    if max_size*batch_size < len(origin_list):
      max_size += 1

    for i in range(max_size):
        o_list = origin_list[i*batch_size:(i+1)*batch_size]
        d_list = destination_list[i*batch_size:(i+1)*batch_size]
...
def process_csv(csv_text):
    ...
    for i, row in enumerate(csv_reader):
        if i==0:
          ...
          origin_list.append(row[start_idx])
          destination_list.append(row[end_idx])
    return headers, origin_list, destination_list

def process_csv_reverse(csv_text, data, mode):
    # Create a StringIO object from the CSV text
    csv_file = StringIO(csv_text)
    ...
    for i, row in enumerate(csv_reader):
        if i==0:
            row.extend(['Date(YYYY-MM-DD)', 'Time(HH:MM:SS)', 'Distance', 'Trip Time'])
            ...
            current_time_utc = current_utc_time.time()
            s=current_time_utc.second
            m= current_time_utc.minute
            ...
            if len(str(h)) == 1:
                h = '0'+str(h)
            trip_time = f'{h}:{m}:{s}'
            row.extend([str(current_date_utc), trip_time])
            new_row = row.extend(data[i-1])
            # row[1] = mode
            data_list.append(row)
    return data_list
...
def lambda_handler(event, context):
    ...
    list_data = process_csv_reverse(text, data, transport_mode)
